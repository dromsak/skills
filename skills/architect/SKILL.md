...
1. **Look for `.claude/architect.md`** in the target repo. If present, it is the profile — it can define a **surface manifest** (named scope → path globs), a one-paragraph **project context / constraints** (e.g. "deliberately monolithic — don't recommend microservices"), **drift rules** (the layering invariants for lens ⑧), **language-idiom** notes (lens ⑭), and **UI conventions** (the design system / component library / token source for lens ⑯). Everything in it is optional, and it's self-describing — parse what's there. The authoring spec lives in `profile-format.md` in this skill's directory; Read it only when offering to create or explain a profile. Absent ⇒ zero-config mode.
...
**Frontend (auto-enabled when the scope contains UI code — components, pages, templates, styles):** ⑯ UI-primitive bypass — the signature smell of pages grown one prompt at a time, each built in isolation, none reusing the last: bespoke one-off components where the project's component library / design system / an existing shared component already provides the primitive (a hand-styled `<button>` next to an existing `<Button>` — *name the existing component*); hardcoded design values (hex colors, px spacing, ad-hoc font sizes) where theme tokens / CSS variables / the Tailwind scale exist; the same UI pattern (modal, form, table, toast) implemented N divergent ways across pages; copy-pasted page markup that should be one shared component; hand-rolled form/fetch/format logic a frontend dependency already in the manifest covers. Where the repo has *no* primitive for a repeated pattern, the finding is the shared component/token to **extract**. **Per-repo design-system notes (which library, where tokens live) come from the profile if it has them.**
...
- **Small target** (a `file`, or a module < ~2k lines): audit **inline** — read the files yourself, apply the lenses, then do a skeptic pass on each finding (re-check the exact lines; for findings ⑭/①/⑯ re-verify the named library/API/in-repo component actually exists and does what's claimed — empirically where a cheap test settles it). Then send the full finding list to **one Sonnet batch-skeptic subagent** (plain `Agent` tool, default-to-refuted, same contract as the workflow's verifier) — self-review can't catch your own motivated reasoning. Drop what it refutes into the report's "Refuted / skipped" section. No heavy machinery beyond that single agent.
- **Surface target** (a whole scope, or > ~2k lines): run the fan-out **workflow** — **frugal by design** (1 reviewer + 1 batch-verifier per chunk = ~2 Sonnet agents/chunk, fixed regardless of how much it finds):
  ...
  2. Run it: `Workflow({ scriptPath: "<this-skill-dir>/audit-workflow.js", args: { scope, chunks: [{label, files:[...]}], lang, projectContext, driftGuide, idiomGuide, uiGuide, schemaScope, uiScope } })` — pass `lang`/`projectContext`/`driftGuide`/`idiomGuide`/`uiGuide` assembled from §0 (empty strings are fine; the script falls back to language-neutral defaults), and `schemaScope: true` only for a persistence-layer scope. The frontend lens ⑯ auto-enables per chunk when its files have UI extensions (`.tsx`/`.jsx`/`.vue`/`.svelte`/`.html`/`.css`/…); pass `uiScope: true` to force it when the frontend lives in plain `.js`/`.ts` files.
...
- `audit-workflow.js` — the frugal fan-out engine for surface-scale targets (per chunk: 1 all-lens reviewer → 1 batch-verifier, all Sonnet; ~2 agents/chunk, fixed cost, hard-capped at `MAX_CHUNKS = 12` ≈ 24 agents). Language-agnostic: takes `lang`/`projectContext`/`driftGuide`/`idiomGuide`/`uiGuide`/`schemaScope`/`uiScope` from the caller (the frontend lens also auto-detects per chunk from file extensions). Returns confirmed, refuted, **and** unverified findings plus the run's real `agents_spawned`/`tokens_out`.
