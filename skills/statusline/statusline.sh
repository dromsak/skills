#!/usr/bin/env bash
#
# Forked from https://github.com/Astro-Han/claude-pace (claude-pace.sh, v0.9.1)
# by yuhan (Astro-Han). MIT-licensed. See the repo-root LICENSE. Local
# modifications by dromsak diverge from upstream — fixes belong here, not
# necessarily upstream.
#
...
PN="${DIR##*/}"
# Windows: project_dir is back-slashed and @tsv doubles each '\' to '\\', so the
# '*/' strip above is a no-op there. Also strip to the last back-slash.
PN="${PN##*\\}"
IS_WT=0 _REPO=""
